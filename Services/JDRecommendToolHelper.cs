...
public class JDRecommendToolHelper
{
        ...
        int count)
    {
        ...
        sb.AppendLine($"为您找到 {products.Count} 款「{keyword}」推荐商品：\n");
        ...
        for (int i = 0; i < products.Count; i++)
        {
            ...
            {
                sb.AppendLine($"店铺: {product.ShopName}");
            }

            // 销售信息
            sb.AppendLine($"30天销量: {FormatNumber(product.SalesCount)}");
            sb.AppendLine($"好评率: {product.GoodRate}%");
            ...
            // 佣金信息（可选：是否显示给用户）
            sb.AppendLine($"预估佣金: ¥{product.Commission:F2} ({product.CommissionRate}%)");
        ...
        sb.AppendLine("💡 **购物提示**：点击\"立即购买\"即可跳转京东购买，支持联盟返佣。");
