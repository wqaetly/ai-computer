...
namespace AiComputer.Services.OCR
{
    ...
    /// 负责检测图像中的文本区域
    /// </summary>
    internal class DbNet : IDisposable
    {
        ...
        public void InitModel(string path, int numThread)
        {
            try
            {
                ...
                inputNames = dbNet.InputNames.ToList();
            }
            catch (Exception ex)
            {
        ...
        {
            if (dbNet == null)
                throw new InvalidOperationException("DbNet not initialized");
            ...
            try
            {
                using (IDisposableReadOnlyCollection<DisposableNamedOnnxValue> results = dbNet.Run(inputs))
                {
                    ...
                    Console.WriteLine("DbNet detection completed");
        ...
        private static List<TextBox> GetTextBoxes(DisposableNamedOnnxValue[] outputTensor, int rows, int cols, ScaleParam s, float boxScoreThresh, float boxThresh, float unClipRatio)
        {
            ...
            Mat dilateElement = new Mat(2, 2, DepthType.Cv8U, 1);
            dilateElement.SetTo(new MCvScalar(1));
        ...
        private static List<Point> Unclip(List<PointF> box, float unclip_ratio)
        {
            ...
            PathsD solution = Clipper.InflatePaths(paths, distance, JoinType.Round, EndType.Polygon);
            ...
            foreach (PointD pt in solution[0])
            {
                retPts.Add(new Point((int)pt.x, (int)pt.y));
            }
