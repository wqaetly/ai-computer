...
namespace AiComputer.Services.OCR
{
    /// <summary>
    /// AngleNet - 角度检测网络
    /// 负责检测文本的方向角度（正向或180度倒转）
    /// </summary>
    internal class AngleNet : IDisposable
    {
        ...
        public void InitModel(string path, int numThread)
        {
            try
            {
                ...
                inputNames = angleNet.InputNames.ToList();
            }
            catch (Exception ex)
            {
        ...
        {
            if (angleNet == null)
                throw new InvalidOperationException("AngleNet not initialized");
